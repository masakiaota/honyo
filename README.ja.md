...
- **Apple Silicon搭載Macでオフライン翻訳する**：初回に約1.13GBのモデルを取得すると、アカウントやAPIキーなしで翻訳できる。訳文は生成に合わせて順次表示する。
...
英語と日本語のオフライン翻訳には、Apple Silicon搭載Macと初回のモデルダウンロードが必要だ。クラウドで翻訳する場合は、インターネット接続と次のいずれかの接続方法を使う。
...
クラウドで翻訳する場合は、原文と追加した翻訳指示を選択中のAIサービスへ送信する。送信した情報には、そのサービスのデータ取り扱い方針が適用される。オフライン翻訳の原文は、このMacの中で処理する。
...
1. **Settings → Offline Translation** を開き、**Download (1.13 GB)** を選ぶ。
2. ダウンロードと検証が完了したら、**Use this model** を選ぶ。英語と日本語の翻訳、およびポップアップ表示へ切り替わり、設定がすぐに保存される。
3. 設定画面の入力欄で試すか、通常どおり文章を選んでコピー操作を二度行う。

モデルは **Hy-MT2 1.8B Q4_K_M** を使い、llama.cppとMetalで処理する。選択中はモデルをメモリに保持し、アプリの起動時にも読み込む。クラウドモデルへの切り替えやモデルの削除で解放する。翻訳ごとに文脈を破棄し、入力は2,000文字、生成時間は90秒までに制限する。同時に翻訳する文章は1件とし、CPUでの推論は最大2スレッドを使う。

モデルは `~/Library/Application Support/Honyo/models` に保存する。Gitやアプリ本体には含めない。取得元の版を固定し、SHA-256で内容を検証する。設定画面からダウンロードの中止、再試行、モデルの削除ができる。中止したダウンロードは最初から取得し直す。Pythonの導入や、利用者によるモデル変換は不要だ。

General設定でストリーミングを有効にすると、ポップアップに訳文を順次表示する。追加の翻訳指示と指示文のAI生成には対応していない。専門用語や曖昧な文章には誤訳が残る。空の出力、繰り返し、途中で切れた訳文、コードやURLの欠落、一部の言語違いは検出するが、意味の正確さまでは保証しない。[モデルの比較結果と実際の訳文](docs/local-translation.md)も参照できる。
